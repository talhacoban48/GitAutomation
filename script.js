// Custom Select Implementation
    class CustomSelect {
      constructor(selectEl, dropdownEl, options, editable = true, storageKey = null) {
    ...
    document.getElementById('gitForm').addEventListener('submit', (e) => {
      ...
      if (!issueNumber || issueNumber < 1) {
        showError('issueNumber', 'Issue number is required and must be a positive number.');
        isValid = false;
      }
      
      // Validate branch name (optional but if provided must be valid)
      if (branchName && !validateGitBranchName(branchName)) {
        showError('branchName', 'Invalid branch name. Could not sanitize to GitHub rules.');
        isValid = false;
      }
      
      // Validate commit message
      if (!validateCommitMessage(commitMessage)) {
        showError('commitMessage', 'Commit message is required and must be max 72 characters.');
        isValid = false;
      }
